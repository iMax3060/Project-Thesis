...
\end{@empty}
    ...
    The counters return values from $1$ to $\texttt{blockCount} - 1$---the range of the buffer frame indexes.
...
\begin{@empty}
    Figure \ref{fig:loop_server_performance} shows the LOOP index generation performance of the NUMA system A---figure \ref{fig:loop_desktop_performance} shows the performance of the UMA system B. The LOOP index generation throughput is total number of indexes generated (from all working threads) per time.
        ...
        \resizebox{\textwidth}{!}{
            \begin{tikzpicture}[]
                \begin{axis}[xlabel = {Number of threads},
                    ...
                    \addplot[mutex]       table[x = threads, y = throughput] {./data/loop/server_mutex_counter.csv};        \label{pgfplots:server-mutex}
                    \addplot[spinlock]    table[x = threads, y = throughput] {./data/loop/server_spinlock_counter.csv};     \label{pgfplots:server-spinlock}
                    \addplot[modulo]      table[x = threads, y = throughput] {./data/loop/server_modulo_counter.csv};       \label{pgfplots:server-modulo}
                    \addplot[local]       table[x = threads, y = throughput] {./data/loop/server_local_counter.csv};        \label{pgfplots:server-local}
                    \addplot[localModulo] table[x = threads, y = throughput] {./data/loop/server_local_modulo_counter.csv}; \label{pgfplots:server-local-modulo}
                    \addplot[lockFree]    table[x = threads, y = throughput] {./data/loop/server_lock_free_counter.csv};    \label{pgfplots:server-lock-free}
                \end{axis}
            \end{tikzpicture}
        }
    ...
    \begin{figure}[ht!]
        ...
        \caption{The throughput of index generations of the evaluated LOOP implementations on system B}
